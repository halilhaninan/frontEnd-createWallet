...
import walletlistData from "./walletlistData";

const MainPage = ({}) => {
  const [walletlistd, setWalletlistd] = useState([]);

  const randomNumberGenerate = () => {
    const tempArray = [];
    const walletListKeys = Object.keys(walletlistData);
    for (let i = 0; i < 5; i++) {
      tempArray.push(Math.floor(Math.random() * 50));
    }
    const updateArray = tempArray.map((x) => walletListKeys[x]);
    console.log(updateArray);
    setWalletlistd(updateArray);
  };
          ...
          <div className="cold-md-2 pt-5">
            ...
            <div id="screen">
              <div id="walletlistterminal" className="col-md-12 row">
                ...
                {walletlistd.map((item) => (
                  <div className="row">
                    <p id="wallet" className="font text-white">
                      <strong className="font text-success"> Wallet: </strong>
                      {item}
                    </p>
                    <p className="font text-white" id="wallet">
                      <strong className="font text-danger"> Pwkey: </strong>
                      {walletlistData[item].secretKey}
                    </p>
                  </div>
                ))}
              </div>
